      ...
      if (snapshot.val()) {
          ...
          if(renderobj){
            const arr = Object.keys(renderobj).map(i => {
              renderobj[i].myEarning = (renderobj[i].customerPaid - renderobj[i].driverShare).toFixed(2)
              renderobj[i].customerPaid = renderobj[i].customerPaid.toFixed(2)
              renderobj[i].rideCost =  (renderobj[i].tripCost - renderobj[i].convenienceFee).toFixed(2)
              renderobj[i].driverShare = renderobj[i].driverShare.toFixed(2)
              console.log( renderobj[i])
              return renderobj[i]
            })
            dispatch({
